...
const Model = require('../models/nomenclature');
const ModelHistory = mongoose.model('nomenclature-history', dataSchemaHistory);
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(data.map(s => ({
      id: s._id, name: s.name, code: s.code, shortName: s.shortName, weight: s.weight
    })));
  }
  catch (error) {
