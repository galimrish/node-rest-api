...
const Model = require('../models/print-info-type');
const ModelHistory = mongoose.model('print-info-type-history', dataSchemaHistory);
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(data.map(s => ({
      id: s._id, name: s.name, code: s.code, shortName: s.shortName
    })));
  }
  catch (error) {
